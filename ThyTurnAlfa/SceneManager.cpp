...
void SceneManager::setupGame() {
    ...
    if (m_availiblePlayers.at(m_curPlyIdx_1) == "Human") {
        ...
        m_players[0] = std::make_unique<AI>(m_availibleCharacters, m_curChrIds_1, &m_treeDepths[0], &m_queue);
    }
    // player 2
    if (m_availiblePlayers.at(m_curPlyIdx_2) == "Human") {
        ...
        m_players[1] = std::make_unique<AI>(m_availibleCharacters, m_curChrIds_2, &m_treeDepths[1], &m_queue);
    }
}
...
void SceneManager::pauseAudio() {
    m_audio.pauseMusic();
}

void SceneManager::resumeAudio() {
    m_audio.resumeMusic();
}
